...
const (
	...
	// PendingTransactionsSubscriptionHashes queries tx hashes for pending
	// transactions entering the pending state
	PendingTransactionsSubscriptionHashes
...
// SubscribePendingTxHashes creates a subscription that writes transaction hashes for
// transactions that enter the transaction pool.
func (es *EventSystem) SubscribePendingTxHashes(hashes chan []common.Hash) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
		typ:       PendingTransactionsSubscriptionHashes,
		created:   time.Now(),
		logs:      make(chan []*types.Log),
		hashes:    hashes,
		txs:       make(chan types.Transactions),
...
func (es *EventSystem) SubscribePendingTxs(txs chan types.Transactions) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) handleTxsEvent(filters filterIndex, ev core.NewTxsEvent) {
	...
	tsx := make(types.Transactions, 0, len(ev.Txs))

	for _, tx := range ev.Txs {
		hashes = append(hashes, tx.Hash())
		tsx = append(tsx, tx)
	}
	for _, f := range filters[PendingTransactionsSubscriptionHashes] {
		f.hashes <- hashes
	}
